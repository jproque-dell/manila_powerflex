...
class PowerFlexTest(test.TestCase):
    ...
    @mock.patch(
        'manila.share.drivers.dell_emc.plugins.powerflex.object_manager.StorageObjectManager',
        autospec=True)
    ...
    def test_connect(self, mock_powerflex_manager):
        ...
        self.assertEqual(expected_erify_certificates,
                         storage_connection.verify_certificates)

    def test_create_share_nfs(self):
        self._mock_powerflex_manager.get_storage_pool_id.return_value = self.STORAGE_POOL_ID
        self._mock_powerflex_manager.create_filesystem.return_value = self.FILESYSTEM_ID
        self._mock_powerflex_manager.create_nfs_export.return_value = self.NFS_EXPORT_ID
        self._mock_powerflex_manager.get_nfs_export_name.return_value = self.NFS_EXPORT_NAME
        ...
        expected_location = '%s:/%s' % (self.POWERFLEX_ADDR,
        ...
        self._mock_powerflex_manager.create_filesystem.assert_called_with(
            self.STORAGE_POOL_ID, self._mock_config.safe_get('dell_nas_server'), self.SHARE_NAME, 8 * units.Gi)
    ...
    def test_create_share_invalid_share_size(self):
        share = {"name": self.SHARE_NAME, "share_proto": 'NFS', "size": 1}

        self.assertRaises(
            exception.ShareBackendException, self.storage_connection.create_share,
            self.mock_context, share, share_server=None)
    ...
    def test_create_snapshot(self):
        self._mock_powerflex_manager.get_fsid_from_export_name.return_value = self.FILESYSTEM_ID
        self._mock_powerflex_manager.create_snapshot.return_value = True

        snapshot = {'name': self.SNAPSHOT_NAME,
                    'share_name': self.SNAPSHOT_PATH, 'id': self.SNAPSHOT_ID}
        self.storage_connection.create_snapshot(
            self.mock_context, snapshot, None)

        # verify the create snapshot API call is executed
        self._mock_powerflex_manager.get_fsid_from_export_name.assert_called_with(
            self.SNAPSHOT_PATH)
        self._mock_powerflex_manager.create_snapshot.assert_called_with(self.SNAPSHOT_NAME,
                                                                        self.FILESYSTEM_ID)
    ...
    def test_delete_share_nfs(self):
        ...
        self._mock_powerflex_manager.get_filesystem_id.return_value = self.FILESYSTEM_ID
    ...
    def test_delete_snapshot(self):
        self._mock_powerflex_manager.get_fsid_from_snapshot_name.return_value = self.FILESYSTEM_ID
        ...
        self._mock_powerflex_manager.get_fsid_from_snapshot_name.assert_called_with(
            self.SNAPSHOT_NAME)
        self._mock_powerflex_manager.delete_filesystem.assert_called_with(
            self.FILESYSTEM_ID)
    ...
    def test_extend_share(self):
        ...
        share = {"name": self.SHARE_NAME,
                 "share_proto": 'NFS', "size": new_share_size}
        self._mock_powerflex_manager.get_filesystem_id.return_value = self.FILESYSTEM_ID
        self.assertFalse(self._mock_powerflex_manager.get_filesystem_id.called)
    ...
    def test_update_access_add_nfs(self):
        ...
        self._mock_powerflex_manager.get_nfs_export_id.return_value = self.NFS_EXPORT_ID
        ...
        self.storage_connection.update_access(
            self.mock_context, share, access_rules, add_rules=None,
    ...
    def test_update_access_invalid_share_protocol(self):
        ...
        self.assertRaises(
            exception.InvalidShare, self.storage_connection.update_access,
            self.mock_context, share, access_rules=None, add_rules=None, delete_rules=None, share_server=None)
    ...
    def test_update_access_add_nfs_invalid_acess_type(self):
        ...
        self.assertRaises(
            exception.InvalidShareAccess, self.storage_connection.update_access,
            self.mock_context, share, access_rules, add_rules=None, delete_rules=None, share_server=None)
    ...
    def test_update_access_add_nfs_backend_failure(self):
        ...
        self.assertRaises(
            exception.ShareBackendException, self.storage_connection.update_access,
            self.mock_context, share, access_rules, add_rules=None, delete_rules=None, share_server=None)

    def test_create_share_from_snapshot(self):
        self.assertRaises(NotImplementedError, self.storage_connection.create_share_from_snapshot,
                          self.mock_context, share=None, snapshot=None)

    def test_allow_access(self):
        self.assertRaises(NotImplementedError, self.storage_connection.allow_access,
                          self.mock_context, share=None, access=None, share_server=None)

    def test_deny_access(self):
        self.assertRaises(NotImplementedError, self.storage_connection.deny_access,
                          self.mock_context, share=None, access=None, share_server=None)

    def test_setup_server(self):
        self.assertRaises(
            NotImplementedError, self.storage_connection.setup_server, network_info=None)

    def test_teardown_server(self):
        self.assertRaises(
            NotImplementedError, self.storage_connection.teardown_server, server_details=None)
